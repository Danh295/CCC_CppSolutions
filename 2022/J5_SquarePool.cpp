    ...
    for (int first : y_vals) { // iterate through the lengths in between the y values of the trees
        for (int second : y_vals) { // x should track the first (smaller) y value, while y tracks the second (greater) y value
            ...
            for (auto t : trees) { // iterate through the tree coordinates
                ...
                ans = std::max(std::min(second - first - 1, t.first - prev - 1), ans); // y - x - 1 gives the valid horizontal side length between the trees, t.first - prev - 1 gives the valid vertical side length 
            ...
            ans = std::max(std::min(second - first - 1, N - prev), ans); // same as above, except the formula changes to N - prev
        }
    }
